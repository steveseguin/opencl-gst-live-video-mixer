...
class another_input_stream:
	...
	def decoder_callback(self, decoder, pad):
		...
		if structure_name.startswith("video"):
			...
			return False
			# we should attach audio stream to audioadder element of output pipeline
			# for simplicity currently, I'm just pushing to a fake endpoint
...
def create_entry(o):
	...
	if m not in stream_srcs:
		...
		stream_info = stream_srcs[m]
		if (stream_srcs[m]['action'] == "preload"):
			stream_info['status'] = "connected"  ## what about error-flagged stream
		else:
...
CLI = 'appsrc name=mysource format=TIME do-timestamp=TRUE is-live=TRUE caps="video/x-raw,format=BGR,width='+str(widthout)+',height='+str(heightout)+',framerate=(fraction)'+str(fpsout)+'/1,pixel-aspect-ratio=(fraction)1/1" ! timeoverlay halignment=right valignment=bottom text="Stream time:" shaded-background=true font-desc="Sans, 32" ! videoconvert ! videoscale ! videorate ! queue ! capsfilter caps="video/x-raw,format=I420,width='+str(widthout)+',height='+str(heightout)+',framerate=(fraction)30/1" ! queue leaky=1 ! tee name=RAW RAW. ! queue ! autovideosink sync=false RAW. ! queue ! x264enc cabac=false aud=true tune=zerolatency byte-stream=false sliced-threads=true threads=4 speed-preset=1 bitrate=4000 key-int-max=20 bframes=0 ! queue ! h264parse ! queue ! video/x-h264,profile=main ! queue ! mux. audiotestsrc name=testtone freq=100 volume=0.0 ! audio/x-raw,format=S8,channels=2,rate=44100 ! audiorate ! audioconvert ! audioresample ! adder name=audioadder is-live=true  ! audioconvert ! audioresample ! queue ! voaacenc bitrate=128000 ! aacparse ! audio/mpeg,mpegversion=4,stream-format=raw ! flvmux streamable=true name=mux ! rtmpsink location="rtmp://a.rtmp.youtube.com/live2/x/steve.hvcd-ssyb-esqr-2x2m" sync=false'
